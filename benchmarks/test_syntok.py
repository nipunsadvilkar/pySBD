...
def syntok_segmenter(text):
    ...
    return [sent for sent in make_sentences(result)]

@pytest.mark.parametrize('text,expected_sents', GOLDEN_EN_RULES_TEST_CASES)
def test_en_sbd_syntok(text, expected_sents):
    """SBD tests from Pragmatic Segmenter"""
    segments = syntok_segmenter(text)
    assert segments == expected_sents


if __name__ == "__main__":
    syntok_segmenter("i. This goes first. ii. And here thereafter.")
